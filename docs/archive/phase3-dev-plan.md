...
| 3-C | line_chart, bar_chart | ✅ **完了** |
| 3-C | Chart.jsプラグイン | ❌ 未実装（優先度低） |
...
## Phase 3-C: チャート機能（オプショナル依存）✅ 完了

### 10. kt.line_chart() ✅ 実装済み

SVGベースで実装。複数データフォーマット対応（配列、オブジェクト配列、2D配列、カラムナー）。
詳細は `docs/design/line-chart-api.md` を参照。

---

### 11. kt.bar_chart() ✅ 実装済み

SVGベースで実装。単一/複数シリーズ、積み上げ/グループ化、横向き、ソート対応。
