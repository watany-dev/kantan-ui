...
| 優先度 | 件数 | 主な内容 |
|--------|------|----------|
| 🔴 高 | 4件 | コード重複、未使用コード、メモリリーク |
| 🟡 中 | 5件 | グローバル状態、型安全性、エラーハンドリング |
| 🟢 低 | 2件 | 型不整合、命名規則 |
...
### Phase 1: 即時対応
1. [ ] `escapeHtml` の共通化 (`src/utils/escape.ts`)
2. [ ] 未使用コードの削除 (`connections` 関連)
3. [ ] JSONパースのエラーハンドリング追加

### Phase 2: 短期改善
4. [ ] ウィジェットロジックのリファクタリング（宣言的APIが命令型をラップ）
5. [ ] セッションクリーンアップのスケジューリング実装
6. [ ] E2Eテストの追加
