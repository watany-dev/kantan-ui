...
| # | 内容 | 主な成果物 | テスト数目安 |
|---|------|-----------|-------------|
| 1 | 型定義 + カラーパレット | `types.ts`, `colors.ts` | 8-10 |
| 2 | データ正規化（共通パイプライン） | `normalize.ts` | 12-15 |
| 3 | 軸スケール計算 | `scale.ts` | 6-8 |
| 4 | BarChart ショートハンド正規化 + ソート | `bar-chart.ts`（前半） | 8-10 |
| 5 | SVG描画（単一シリーズ縦棒） | `bar-chart.ts`（描画） | 8-10 |
| 6 | 複数シリーズ（グループ化 + 積み上げ） | `bar-chart.ts`（マルチ） | 8-10 |
| 7 | 横向きバー（horizontal） | `bar-chart.ts`（横向き） | 4-6 |
| 8 | kt.bar_chart 統合 + CSS + エクスポート | `charts.ts`, `index.ts`, `default.ts` | 6-8 |
| 9 | エッジケース + バリデーション | テスト追加 | 8-10 |
