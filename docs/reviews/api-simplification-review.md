...
| Phase | 内容 | 結果 |
|-------|------|------|
| Phase 1 | chart/shared.ts に共通関数を抽出 | **-95行** (228追加 / 323削除) |
| Phase 2 | scale.ts に niceScale 統一、colors.ts 統一 | **-52行** (54追加 / 106削除) |
| Phase 3 | renderLineChart() 分割 | **スキップ** - 再評価の結果、既に十分に分割済みと判断 |
| Phase 4 | bar-chart 縦横レンダリング統合 | **-43行** (113追加 / 156削除) |

### 合計: -190行（585削除）、全2667テストパス
...
|------|--------|--------|------|
| 重複コード | 約400行 | 0行 | ほぼ0行 |
| 合計行数 | 約3,175行 | 約2,750行 | 約2,985行 (-190行) |
| 同一ロジックの実装数 | 2〜3箇所 | 1箇所 | 1箇所 |
