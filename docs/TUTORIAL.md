...
`kt.area_chart()`は折れ線グラフの下部を塗りつぶした「エリアチャート」を表示します。量の変化や累積値を視覚的に強調するのに適しています。
...
オブジェクト配列を使うと、複数シリーズを重ねて描画します（デフォルトは重ね表示）:
...
タイトルやラベル、カスタムカラーも指定できます:

```typescript
kt.area_chart(data, {
  x: "month",
  y: ["売上", "コスト"],
  color: ["#FF000080", "#0000FF80"],  // rgba/8桁HEXで透明度を指定可能
  x_label: "月",
  y_label: "金額（円）",
  title: "売上とコストの推移",
  height: 300,
...
| `x_label` | `string` | - | x軸ラベル |
| `y_label` | `string` | - | y軸ラベル |
| `color` | `string \| string[]` | パレット | エリアの色（Tableau 10 パレット） |
| `height` | `number` | `400` | グラフの高さ（px） |
| `stack` | `boolean` | `false` | 複数シリーズを積み上げる（falseで重ね表示） |
| `use_container_width` | `boolean` | `true` | コンテナ幅に合わせる |
| `title` | `string` | - | グラフタイトル |
