...
> **✅ 実装完了** (2026-02-08)
>
> 全6イテレーション（Iteration 0〜5）完了。単一シリーズ、複数シリーズ（重ね表示）、
> 積み上げモード、エッジケース・バリデーション・セキュリティ対策を含む。
> `kt.area_chart()` として宣言的API統合済み。CSSスタイル追加済み。
...
### 実装前
- [x] `src/kt/chart/normalize.ts` の `normalizeChartData()` のインターフェース確認
- [x] `src/widgets/line-chart.ts` から抽出可能な描画関数の洗い出し
- [x] CSS クラス命名規則の確認（`kt-chart-*` 共通 vs `kt-area-chart-*` 固有）
...
- [x] アクセシビリティ属性が正しく設定されている
- [x] レスポンシブ表示が動作する
- [x] `number[]` ショートハンドが動作する
- [x] 単一シリーズのエリアチャートが正しく描画される
- [x] 複数シリーズの重ね表示が正しく描画される
- [x] 積み上げエリアチャートが正しく描画される
- [x] null値での分断が正しく動作する
- [x] ユーザー指定の透明度が尊重される
- [x] 共通描画ユーティリティ抽出後も line_chart/bar_chart の既存テストがパスする
