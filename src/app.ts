...
import { createWebSocketAdapter, createWebSocketAdapterAsync } from "./websocket";
...
/**
 * Deno対応の非同期版ファクトリー関数
 * WebSocketアダプターの初期化に動的インポートを使用
 *
 * @example
 * ```typescript
 * // Deno
 * const kantanApp = await createAppAsync((kt) => {
 *   kt.write("Hello from Deno!");
 * });
 * Deno.serve({ port: 3000 }, kantanApp.fetch);
 * ```
 */
export async function createAppAsync(
	script: Script,
	options?: KantanAppOptions,
): Promise<KantanApp> {
	const { port, hostname, ...userConfig } = options ?? {};
	const config = resolveConfig(userConfig);
	const sessionManager = new SessionManager(config.session, config.security);
